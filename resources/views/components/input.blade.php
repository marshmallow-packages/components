...
<div class="relative w-full">
    @if (!$hideLabel)
        <x-mm-tooltip-label :required="$required" for="{{ $attributes['id'] }}"
    ...
    <div class="relative w-full">
        ...
        {{-- pl-7 pr-12 --}}
        @if ($defer)
            <x-mm-default-input :required="$required" :id="$attributes['id']" :x-ref="$attributes['x-ref']" :type="$attributes['type']"
                ...
                :disabled="$disabled" />
        @elseif($lazy)
            <x-mm-default-input :required="$required" :id="$attributes['id']" :x-ref="$attributes['x-ref']" :class="$class"
                :step="$step" :min="$min" :max="$max" :type="$attributes['type']" :name="$attributes['name']"
                :value="$value" :autofocus="$autofocus" :type="$type" wire:model.lazy="{{ $attributes['name'] }}"
                :placeholder="$placeholder" :autocomplete="$attributes['autocomplete']" :readonly="$readonly" :disabled="$disabled" />
        @else
            <x-mm-default-input :required="$required" :id="$attributes['id']" :type="$attributes['type']" :class="$class"
                :step="$step" :min="$min" :max="$max" :name="$attributes['name']" :value="$value"
                :autofocus="$autofocus" :type="$type" wire:model="{{ $attributes['name'] }}" :placeholder="$placeholder"
                :autocomplete="$attributes['autocomplete']" :readonly="$readonly" :disabled="$disabled" />
        @endif
    ...
    <x-mm-error for="{{ $attributes['id'] }}" :icon="false" />
</div>
