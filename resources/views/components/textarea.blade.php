...
if ($placeholder && $attributes->has('placeholder')) {
...
$class = 'border-gray-300 focus:border-gold-300 focus:ring  text-[0.9rem] focus:ring-gold-200 focus:ring-opacity-50 rounded-md shadow-sm w-full min-h-10 placeholder-white-600 bg-transparent border p-3  placeholder-text-md peer focus:outline-none  read-only:italic read-only:bg-gray-300/20 disabled:italic disabled:bg-gray-300/20';
...
maxCount = $refs.countme.maxLength">
    ...
    <x-mm-tooltip-label class="mb-1 ml-1" :required="$required" for="{{ $attributes['id'] }}" :question="$question" />
